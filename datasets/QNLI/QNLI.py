...
class QNLI(datasets.GeneratorBasedBuilder):
    def _info(self):
        return datasets.DatasetInfo(
            features=datasets.Features({
                ...
                'labels': datasets.features.ClassLabel(names=['entailment', 'not_entailment'])
            }),
    ...
    def _split_generators(self, dl_manager):
        return [
            datasets.SplitGenerator(name=datasets.Split(f'{split}_{seed}'), gen_kwargs={"filepath": f'./datasets/QNLI/{seed}/{split}.tsv'})
